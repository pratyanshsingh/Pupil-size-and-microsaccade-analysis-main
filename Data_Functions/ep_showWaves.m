...
%             end
        ...
        if (numWaves==1) && ~any(ismember({'STS';'BSC'},EPdataset.dataset(EPmain.view.dataset(iColor)).cellTypes{theCell}))
            ...
            EPwaves.normCells=[EPwaves.normCells; 0];
        end
        if ~isempty(tempDataImag)
...
%                         end
                    ...
                    if any(EPwaves.STSdata==EPwaves.colorIndex(iWave)) && (iWave>1) && (iWave<length(waveList)) && EPwaves.normCells(iWave-1) && EPwaves.normCells(iWave+1)
                        ...
                        if ~isempty(breakList)
                            theData1=squeeze(EPwaves.totalData(iChan,:,waveList(iWave-1),:));
                            theData2=squeeze(EPwaves.totalData(iChan,:,waveList(iWave+1),:));
