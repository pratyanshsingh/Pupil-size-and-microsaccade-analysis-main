...
if strcmp('EPtopos',theFunction)
    ...
    if any(EPmain.view.rel(EPmain.view.dataset(1:EPmain.numColors) <= length(EPdataset.dataset)))
        if strcmp(functionData.type,'freq')
            if EPtopos.complexData && rem(row,2)
    ...
    case 'VLT'
        ...
        if (length(functionData.plotColors)-numImages) > 0 %if there will be waveforms
            ...
            for iWave=1:length(waveList)
                ...
                if any(functionData.STSdata==functionData.colorIndex(iWave)) && (iWave>1) && (iWave<length(waveList)) && functionData.normCells(iWave-1) && functionData.normCells(iWave+1)
                    ...
                    if ~isempty(breakList)
                        theData1=squeeze(theData(theChan,:,waveList(iWave-1),:));
                        theData2=squeeze(theData(theChan,:,waveList(iWave+1),:));
        ...
        if (length(functionData.plotColors)-numImages) > 0 %if there will be waveforms
            ...
            for iWave=1:length(waveList)
                ...
                if strcmp('EPtopos',theFunction) && strcmp(EPtopos.type,'freq')
                    plot(1:size(theData,1),squeeze(theData(theChan,:,theWave,:)),'LineStyle',theLineStyle,'color',functionData.thePlotColors(theWave,:),'LineWidth',EPmain.preferences.view.color(theColor).lineSize);
                else
