...
%
% bufix 9/17/20 JD
% Fixed crash when scrolling along the time axis and there are no events marked on screen.
%
% bufix 11/25/22 JD
% Trimming now registered in History records.
%
% bufix 10/1/23 JD
% Fixed crash when saving changes.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
EPmain.handles.trimData = figure('Name', 'Trim Data', 'NumberTitle', 'off', 'Position',[scrsz(1)+EPmain.panesize(1)+70 scrsz(2) scrsz(3)-270 scrsz(4)], 'MenuBar', 'none', 'CloseRequestFcn',@confirmCloseTrim);
colormap jet;
drawnow

EPtrimData.EPdata=ep_loadEPdataset(EPmain.segment.dataset);

if ~strcmp(EPtrimData.EPdata.dataType,'continuous')
    disp('Error - only continuous data may be trimmed.')
    return
end
    ...
    for iEvent=1:length(eventList)
        if ~isempty(EPtrimData.EPdata.events{1}(eventList(iEvent)).value)
            EPtrimData.handles.events(iEvent)=uicontrol('Style','text','HorizontalAlignment','left','String', EPtrimData.EPdata.events{1}(eventList(iEvent)).value,'FontSize',EPmain.fontsize,...
                'Position',[200+((EPmain.scrsz(3)-600)*(EPtrimData.displayLength/(EPtrimData.displayLength+1))/EPtrimData.displayLength)*EPtrimData.EPdata.events{1}(iEvent).sample EPmain.scrsz(4)-100 60 20]);
...
EPtrimData.handles.displayEnd=uicontrol('Style','text','HorizontalAlignment','left','String', ep_ms2time(EPtrimData.displayPoints(end)*(1000/EPtrimData.EPdata.Fs)),'FontSize',EPmain.fontsize,...
    'Position',[EPmain.scrsz(3)-470 160 80 20]);
...
if EPtrimData.edited
    theDescription='Trimmed dataset.';
    EPtrimData.EPdata.history=ep_addHistory(EPtrimData.EPdata.history, EPmain.preferences.records, theDescription, EPmain.handles.hMainWindow, EPmain.preferences, EPmain.preferences.EPver);
    ep_saveEPdataset(EPtrimData.EPdata,EPmain.segment.dataset,'no');
end
...
if isscalar(EPtrimData.displayChans)
...
set(EPtrimData.handles.xSlider,'SliderStep',[EPtrimData.displayLength/EPtrimData.numPoints max(.2,length(EPtrimData.displayLength)/EPtrimData.numPoints)]);
...
pointsOver=max(EPtrimData.displayPoints)-EPtrimData.numPoints;
if ~isempty(pointsOver)
    if pointsOver>EPtrimData.displayPoints(1)
        pointsOver=EPtrimData.displayPoints(1)-1;
    end
    EPtrimData.displayPoints=[EPtrimData.displayPoints(1)-pointsOver:EPtrimData.numPoints];
end
...
if xChange && isfield(EPtrimData.handles,'events')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Confirms closing of the Trim pane.
function confirmCloseTrim(src,event)

global EPtrimData EPmain

if isempty(EPtrimData) || (~EPtrimData.done && EPtrimData.edited)
    selection = questdlg('Close The Trim Window Without Saving Changes?', ...
        'Confirm Closure', ...
        'Yes','No','Yes');
    switch selection
        case 'Yes'
...
if ishandle(EPmain.handles.trimData)
    delete(EPmain.handles.trimData)
else
    delete(gcf)
end
