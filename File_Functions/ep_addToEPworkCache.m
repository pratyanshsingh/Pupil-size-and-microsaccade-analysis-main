...
%
% bufix 11/23/18 JD
% Fixed crash when there are no "EEG" channels.
%
% bugfix 1/13/19 JD
% Recognizes REG channels as well as EEG channels for setting min and max voltages, thus avoiding crashes in the View function.
%
% modified 6/15/19 JD
% Added .inMemory field.
%
% modified 11/5/19 JD
% Added sessNums, sessNames, and subjectSpecs fields.
%
% modified 12/26/19 JD
% Added GAVsubs and isCovAVE.
%
% modified 2/18/20 JD
% Added support for BSC channel type.
%
% modified 3/27/25 JD
% Added lastChange field.
...
EEGchans=find(ismember(EPdata.chanTypes,{'EEG','REG'}));
newDataset.plotMVmin=[];
newDataset.plotMVmax=[];
newDataset.plotMVminAbs=[];
newDataset.plotMVmaxAbs=[];
...
newDataset.sessNames=EPdata.sessNames;
...
newDataset.sessNums=EPdata.sessNums;
...
newDataset.subjectSpecs=EPdata.subjectSpecs;
newDataset.timeNames=EPdata.timeNames;
...
newDataset.GAVsubs=GAVsubs;
newDataset.isCovAVE=~isempty(EPdata.covAVE);
newDataset.isNoise=~isempty(EPdata.noise);
newDataset.inMemory=0;
if ~isempty(EPdata.history)
    newDataset.lastChange=EPdata.history{end,1}.time;
else
    newDataset.lastChange='';
end
