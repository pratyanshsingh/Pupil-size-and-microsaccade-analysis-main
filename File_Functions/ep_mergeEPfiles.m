...
if size(outData.GAVsubs,2)==length(outData.cellNames)
    ...
    for iGAVsub=2:size(outData.GAVsubs,1)
        for iCell=1:size(outData.GAVsubs,2)
            for iRow=1:size(outData.GAVsubs{iGAVsub,iCell,end},1)
                theRow=find(EPdataOut.GAVsubs{theSub,iCell,end}(iRow,1)==EPdataOut.GAVsubs{theSub,1,end}(:,1));
                if ~isempty(theRow)
                    EPdataOut.GAVsubs{theSub,1,end}(theRow,2)=EPdataOut.GAVsubs{theSub,1,end}(theRow,2)+EPdataOut.GAVsubs{theSub,theCell,end}(iRow,2);
                else
                    EPdataOut.GAVsubs{theSub,1,end}(end+1,1)=EPdataOut.GAVsubs{theSub,iCell,end}(:,1);
                    EPdataOut.GAVsubs{theSub,1,end}(end,2)=EPdataOut.GAVsubs{theSub,iCell,end}(:,2);
                end
            end
        end
    end
