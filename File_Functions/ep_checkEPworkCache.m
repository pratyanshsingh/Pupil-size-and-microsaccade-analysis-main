...
%
% bugfix 11/10/22 JD
% Fixed crash when EPdataset has no dataset field yet.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ~isfield(EPdataset,'dataset') || isempty(EPdataset.dataset)
    EPdataset.dataset='';
end
...
%     end
    ...
    if ~isfield(EPdataset.dataset,'subjectSpecs')
        cacheBad=37;
    end
    if ~isfield(EPdataset.dataset,'timeNames')
        cacheBad=27;
    ...
    if ~isfield(EPdataset.dataset,'inMemory')
        for iDataSet=1:length(EPdataset.dataset)
            EPdataset.dataset(iDataSet).inMemory=0;
        end
    end
    if ~isfield(EPdataset.dataset,'sessNames')
        cacheBad=35;
    end
    if ~isfield(EPdataset.dataset,'sessNums')
        cacheBad=36;
    end
    if ~isfield(EPdataset.dataset,'GAVsubs')
        cacheBad=37;
    end
    if ~isfield(EPdataset.dataset,'isCovAVE')
        cacheBad=38;
    end
    if ~isfield(EPdataset.dataset,'isNoise')
        cacheBad=39;
    end
end
...
if cacheBad
    ...
    for iDataSet=1:length(EPdataset.dataset)
        oldEPdata=ep_loadEPdataset(iDataSet,1);
