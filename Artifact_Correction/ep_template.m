...
% Added spike potential to set of templates.
...
% Improved template function's waveform display for the spike potential artifact.
% Moved spike potential file template to separate file.
%
% bugfix & modified 8/9/21 JD
% Fixed crash when the EOG channels are specified in preferences and some of them are indicated as being missing with a -1.
% Added support for canonical 10-05 coordinates in eloc structure.
% Fixed crash when there are channels that are not "EEG" type.
%
% bugfix 12/5/22 JD
% Fixed crash when clicking on topo for 3D image.
%
% modified 9/26/24 JD
% median correct displayed data so more robust with non-baseline corrected segmented data when forming manual templates.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
global EPmain EPdataset EPtemplate EPtictoc
...
for iData=1:length(EPdataset.dataset)
    EPtemplate.EEGchans=find(strcmp('EEG',EPdataset.dataset(iData).chanTypes));
    if any(strcmp(EPdataset.dataset(iData).dataType,{'single_trial','continuous'})) && ~isempty(EPdataset.dataset(iData).eloc) && (length([EPdataset.dataset(iData).eloc(EPtemplate.EEGchans).theta]) == length(EPtemplate.EEGchans)) && (length([EPdataset.dataset(iData).eloc(EPtemplate.EEGchans).radius]) == length(EPtemplate.EEGchans)) && isempty(EPdataset.dataset(iData).freqNames) && isempty(EPdataset.dataset(iData).facNames)
        EPtemplate.sessionFiles=[EPtemplate.sessionFiles iData];
    end
end
...
EPtemplate.windowHeight=600;
EPtemplate.topoSize=80;
...
EPtemplate.handles.window = figure('Name', 'Eye Artifact Templates Creation', 'NumberTitle', 'off', 'Position',[scrsz(1)+EPmain.panesize(1) scrsz(4)-EPtemplate.windowHeight 700 EPtemplate.windowHeight], 'CloseRequestFcn',@confirmCloseTemplate);
colormap jet;
...
ep_tictoc('ioStart');
EPtemplate.EPdata=ep_loadEPdataset(EPtemplate.sessionFiles(EPtemplate.dataset));
ep_tictoc('ioFinish');
...
%baseline correct with median of each trial
templateData=EPtemplate.EPdata;
for iTrial=1:size(templateData.data,3)
    templateData.data(:,:,iTrial)=templateData.data(:,:,iTrial)-repmat(median(templateData.data(:,:,iTrial),2,'omitnan'),1,size(templateData.data,2));
end
[EPtemplate.badChans]=ep_detectBadChans(templateData, [], [], EPtemplate.criteria);

EPtemplate.minVolt=-400;
EPtemplate.maxVolt=400;
EPtemplate.minSacVolt=-400;
EPtemplate.maxSacVolt=400;
EPtemplate.minSPVolt=-100;
EPtemplate.maxSPVolt=100;
...
EPtemplate.handles.dataset = uicontrol('Style','text',...
    'String','LUV','FontSize',EPmain.fontsize,'Position',[5 EPtemplate.windowHeight-250 30 20]);
...
EPtemplate.handles.LUVEOG = uicontrol('Style','popupmenu',...
    ...
    'Value', EPtemplate.VEOG(1),'Position',[35 EPtemplate.windowHeight-250 90 20],...
    'Callback', @changeEOG);

EPtemplate.handles.dataset = uicontrol('Style','text',...
    'String','RUV','FontSize',EPmain.fontsize,'Position',[5 EPtemplate.windowHeight-270 30 20]);

EPtemplate.handles.RUVEOG = uicontrol('Style','popupmenu',...
    'String',EPtemplate.EPdata.chanNames(EPtemplate.EEGchans),'FontSize',EPmain.fontsize,...
    'Value', EPtemplate.VEOG(2),'Position',[35 EPtemplate.windowHeight-270 90 20],...
    'Callback', @changeEOG);

EPtemplate.handles.dataset = uicontrol('Style','text',...
    'String','LLV','FontSize',EPmain.fontsize,'Position',[5 EPtemplate.windowHeight-290 30 20]);

EPtemplate.handles.LLVEOG = uicontrol('Style','popupmenu',...
    'String',EPtemplate.EPdata.chanNames(EPtemplate.EEGchans),'FontSize',EPmain.fontsize,...
    'Value', EPtemplate.VEOG(3),'Position',[35 EPtemplate.windowHeight-290 90 20],...
    'Callback', @changeEOG);

EPtemplate.handles.dataset = uicontrol('Style','text',...
    'String','RLV','FontSize',EPmain.fontsize,'Position',[5 EPtemplate.windowHeight-310 30 20]);

EPtemplate.handles.RLVEOG = uicontrol('Style','popupmenu',...
    'String',EPtemplate.EPdata.chanNames(EPtemplate.EEGchans),'FontSize',EPmain.fontsize,...
    'Value', EPtemplate.VEOG(4),'Position',[35 EPtemplate.windowHeight-310 90 20],...
    'Callback', @changeEOG);

EPtemplate.handles.dataset = uicontrol('Style','text',...
    'String','LH','FontSize',EPmain.fontsize,'Position',[5 EPtemplate.windowHeight-330 30 20]);

EPtemplate.handles.LHEOG = uicontrol('Style','popupmenu',...
    'String',EPtemplate.EPdata.chanNames(EPtemplate.EEGchans),'FontSize',EPmain.fontsize,...
    'Value', EPtemplate.HEOG(1),'Position',[35 EPtemplate.windowHeight-330 90 20],...
    'Callback', @changeEOG);

EPtemplate.handles.dataset = uicontrol('Style','text',...
    'String','RH','FontSize',EPmain.fontsize,'Position',[5 EPtemplate.windowHeight-350 30 20]);

EPtemplate.handles.RHEOG = uicontrol('Style','popupmenu',...
    'String',EPtemplate.EPdata.chanNames(EPtemplate.EEGchans),'FontSize',EPmain.fontsize,...
    'Value', EPtemplate.HEOG(2),'Position',[35 EPtemplate.windowHeight-350 90 20],...
    'Callback', @changeEOG);

EPtemplate.handles.leftTrial = uicontrol('Style', 'pushbutton', 'String', '<--','FontSize',EPmain.fontsize,...
    'Position', [120 EPtemplate.windowHeight-550 50 30], 'Callback', @leftTrial);
...
EPtemplate.handles.rightTrial = uicontrol('Style', 'pushbutton', 'String', '-->','FontSize',EPmain.fontsize,...
    'Position', [330 EPtemplate.windowHeight-550 50 30], 'Callback', @rightTrial);
...
EPtemplate.handles.trialNum=uicontrol('Style','popupmenu',...
    ...
    'Position',[170 EPtemplate.windowHeight-555 160 30],'Callback', @menuTrial);

EPtemplate.handles.leftDataset = uicontrol('Style', 'pushbutton', 'String', '<--','FontSize',EPmain.fontsize,...
    'Position', [120 EPtemplate.windowHeight-580 50 30], 'Callback', @leftDataset);
...
EPtemplate.handles.rightDataset = uicontrol('Style', 'pushbutton', 'String', '-->','FontSize',EPmain.fontsize,...
    'Position', [330 EPtemplate.windowHeight-580 50 30], 'Callback', @rightDataset);
...
EPtemplate.handles.dataset = uicontrol('Style','popupmenu',...
    ...
    'Value', EPtemplate.dataset,'Position',[170 EPtemplate.windowHeight-585 160 30],...
...
[theCzChan, theOrder] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, EPtemplate.badChans, 'Cz', EPtemplate.EPdata.montage);
[thePzChan, theOrder] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, EPtemplate.badChans, 'Pz', EPtemplate.EPdata.montage);
EPtemplate.SPchans=[thePzChan; EPtemplate.VEOG(3); EPtemplate.VEOG(4)];
...
EPtemplate.screenCoords(1,:)=[150 EPtemplate.windowHeight-100 200 EPtemplate.topoSize];
...
EPtemplate.handles.blinkTopo = axes('units','pixels','position',[400 EPtemplate.windowHeight-100 EPtemplate.topoSize EPtemplate.topoSize]);
...
[Xi,Yi,Zi] = griddata(EPtemplate.x(EPtemplate.EEGchans),EPtemplate.y(EPtemplate.EEGchans),EPtemplate.blinks.template,[1:EPtemplate.gridSize]',[1:EPtemplate.gridSize],'linear');

EPtemplate.handles.blink = axes('units','pixels','position',[550 EPtemplate.windowHeight-100 EPtemplate.topoSize EPtemplate.topoSize]);
...
EPtemplate.handles.addBlink = uicontrol('Style', 'pushbutton', 'String', 'Add Blink','FontSize',EPmain.fontsize,...
    'Position', [550 EPtemplate.windowHeight-130 100 30], 'Callback', @addBlink);
...
EPtemplate.handles.blinkNum=uicontrol('Style','text',...
    ...
    'Position',[660 EPtemplate.windowHeight-125 30 20]);

%Waveform plot of H saccade channels
EPtemplate.screenCoords(2,:)=[150 EPtemplate.windowHeight-220 200 EPtemplate.topoSize];
...
EPtemplate.handles.saccadeTopo = axes('units','pixels','position',[400 EPtemplate.windowHeight-220 EPtemplate.topoSize EPtemplate.topoSize]);
...
EPtemplate.handles.saccade = axes('units','pixels','position',[550 EPtemplate.windowHeight-220 EPtemplate.topoSize EPtemplate.topoSize]);
...
EPtemplate.handles.addSaccade = uicontrol('Style', 'pushbutton', 'String', 'Add H Saccade','FontSize',EPmain.fontsize,...
    'Position', [550 EPtemplate.windowHeight-250 100 30], 'Callback', @addSaccade);
...
EPtemplate.handles.saccadeNum=uicontrol('Style','text',...
    ...
    'Position',[660 EPtemplate.windowHeight-245 30 20]);

%Waveform plot of V saccade channels
EPtemplate.screenCoords(3,:)=[150 EPtemplate.windowHeight-340 200 EPtemplate.topoSize];
...
EPtemplate.handles.VsaccadeTopo = axes('units','pixels','position',[400 EPtemplate.windowHeight-340 EPtemplate.topoSize EPtemplate.topoSize]);
...
[Xi,Yi,Zi] = griddata(EPtemplate.x(EPtemplate.EEGchans),EPtemplate.y(EPtemplate.EEGchans),EPtemplate.Vsaccades.template,[1:EPtemplate.gridSize]',[1:EPtemplate.gridSize],'linear');

EPtemplate.handles.Vsaccade = axes('units','pixels','position',[550 EPtemplate.windowHeight-340 EPtemplate.topoSize EPtemplate.topoSize]);
...
EPtemplate.handles.addVSaccade = uicontrol('Style', 'pushbutton', 'String', 'Add V Saccade','FontSize',EPmain.fontsize,...
    'Position', [550 EPtemplate.windowHeight-370 100 30], 'Callback', @addVSaccade);
...
EPtemplate.handles.VsaccadeNum=uicontrol('Style','text',...
    ...
    'Position',[660 EPtemplate.windowHeight-365 30 20]);

%Waveform plot of spike potential channels
EPtemplate.screenCoords(4,:)=[150 EPtemplate.windowHeight-460 200 EPtemplate.topoSize];
EPtemplate.handles.sacPotPlot = axes('units','pixels','position',EPtemplate.screenCoords(4,:));
EPtemplate.handles.sacPotWaves = plot([EPtemplate.startEpoch:EPtemplate.endEpoch],EPtemplate.theTrial(EPtemplate.SPchans,:));
axis([EPtemplate.startEpoch EPtemplate.endEpoch EPtemplate.minSPVolt EPtemplate.maxSPVolt]);
EPtemplate.handles.sacPotMarker=line(repmat(EPtemplate.sacPotMarker,length([EPtemplate.minSPVolt:EPtemplate.maxSPVolt]),1),[EPtemplate.minSPVolt:EPtemplate.maxSPVolt],'Color','red','LineWidth',1); %marker
for iChan=1:length(EPtemplate.handles.sacPotWaves)
    set(EPtemplate.handles.sacPotWaves(iChan),'YDataSource',['EPtemplate.theTrial(' num2str(EPtemplate.SPchans(iChan)) ',:)'])
end

%2D plot of spike potential channels
trialGoodChans=setdiff(EPtemplate.EEGchans,EPtemplate.badChans);
...
EPtemplate.handles.sacPotTopo = axes('units','pixels','position',[400 EPtemplate.windowHeight-460 EPtemplate.topoSize EPtemplate.topoSize]);
EPtemplate.handles.sacPotTopoImage = imagesc(Zi);
set(gca,'XTickLabel','','YTickLabel','');
set(EPtemplate.handles.sacPotTopoImage,'ButtonDownFcn',@D3headPlot);

%2D plot of spike potential template
[Xi,Yi,Zi] = griddata(EPtemplate.x(EPtemplate.EEGchans),EPtemplate.y(EPtemplate.EEGchans),EPtemplate.sacPot.template,[1:EPtemplate.gridSize]',[1:EPtemplate.gridSize],'linear');

EPtemplate.handles.sacPot = axes('units','pixels','position',[550 EPtemplate.windowHeight-460 EPtemplate.topoSize EPtemplate.topoSize]);
EPtemplate.handles.sacPotImage = imagesc(Zi);
set(gca,'XTickLabel','','YTickLabel','');
set(EPtemplate.handles.sacPotImage,'ButtonDownFcn',@D3headPlot);

EPtemplate.handles.addSacPot = uicontrol('Style', 'pushbutton', 'String', 'Add Spike Potentials','FontSize',EPmain.fontsize,...
    'Position', [550 EPtemplate.windowHeight-490 100 30], 'Callback', @addSacPot);
...
EPtemplate.handles.sacPotNum=uicontrol('Style','text',...
    ...
    'Position',[660 EPtemplate.windowHeight-485 30 20]);
...
newData=ep_loadEPdataset(EPtemplate.sessionFiles(EPtemplate.dataset));
...
if newMontage
    ...
    for i=1:length(EPtemplate.handles.sacPotWaves)
        set(EPtemplate.handles.sacPotWaves(i),'YDataSource',['EPtemplate.theTrial(' num2str(EPtemplate.SPchans(i)) ',:)'])
    end
...
[Xi,Yi,Zi] = griddata(EPtemplate.x(EPtemplate.EEGchans),EPtemplate.y(EPtemplate.EEGchans),EPtemplate.saccades.template,[1:EPtemplate.gridSize]',[1:EPtemplate.gridSize],'linear');
...
set(EPtemplate.handles.VsaccadeNum,'String',sprintf('%d',EPtemplate.Vsaccades.num));
...
set(EPtemplate.handles.sacPotNum,'String',sprintf('%d',EPtemplate.sacPot.num));
...
[FileName,PathName,FilterIndex] = uigetfile('*.mat','Load Spike Potential Template','spikePot.mat');
if FileName ~= 0
    eval(['load ''' PathName FileName '''']);
    
    if ~exist('EPspikePot','var')
        msg{1}='Not a spike potential template.';
        [msg]=ep_errorMsg(msg);
        return
    end
    
    if length(EPspikePot.eloc) ~= length(EPtemplate.sacPot.eloc)
        msg{1}=['Number of spike potential electrodes (' num2str(length(EPspikePot.eloc)) ') different from current dataset (' num2str(length(EPtemplate.sacPot.eloc)) ').'];
        [msg]=ep_errorMsg(msg);
        return
    end
    
    if any([EPspikePot.eloc.theta]-[EPtemplate.sacPot.eloc.theta])
        msg{1}='Spike potential electrode locations not consistent with current data.';
        [msg]=ep_errorMsg(msg);
        return
    end
...
if EPtemplate.blinks.num > 0
    [FileName,PathName,FilterIndex] = uiputfile('*.mat','Save Blink Template','blinks.mat');
    if FileName ==0
    ...
    disp('No blink template formed yet.');
end

if (EPtemplate.saccades.num > 0) || (EPtemplate.Vsaccades.num > 0)
    [FileName,PathName,FilterIndex] = uiputfile('*.mat','Save Saccade Template','saccades.mat');
    if FileName ==0
    ...
    disp('No saccade template formed yet.');
end

if (EPtemplate.sacPot.num > 0)
    [FileName,PathName,FilterIndex] = uiputfile('*.mat','Save Spike Potential Template','spikePot.mat');
    if FileName ==0
        ...
        EPspikePot.sacPot.template=EPtemplate.sacPot.template;
        EPspikePot.sacPot.num=EPtemplate.sacPot.num;
        EPspikePot.eloc=EPtemplate.sacPot.eloc;
        eval(['save ''' PathName FileName ''' EPspikePot']);
    end
else
    disp('No saccadic potential template formed yet.');
end
...
set(EPtemplate.handles.blinkNum,'String',sprintf('%d',EPtemplate.blinks.num));
...
set(EPtemplate.handles.saccadeNum,'String',sprintf('%d',EPtemplate.saccades.num));
...
if (length(EPmain.preferences.preprocess.EOGchans) ~=6) || (max(EPmain.preferences.preprocess.EOGchans) > length(EPtemplate.EPdata.eloc))
    [eog.LUVEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'LUVEOG', EPtemplate.EPdata.montage);
    [eog.RUVEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'RUVEOG', EPtemplate.EPdata.montage);
    [eog.LLVEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'LLVEOG', EPtemplate.EPdata.montage);
    [eog.RLVEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'RLVEOG', EPtemplate.EPdata.montage);
    [eog.LHEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'F9', EPtemplate.EPdata.montage);
    [eog.RHEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'F10', EPtemplate.EPdata.montage);
else
    if EPmain.preferences.preprocess.EOGchans(1) ~= -1
        eog.LUVEOG=EPmain.preferences.preprocess.EOGchans(1);
    else
        [eog.LUVEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'LUVEOG', EPtemplate.EPdata.montage);
    end
    if EPmain.preferences.preprocess.EOGchans(2) ~= -1
        eog.RUVEOG=EPmain.preferences.preprocess.EOGchans(2);
    else
        [eog.RUVEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'RUVEOG', EPtemplate.EPdata.montage);
    end
    if EPmain.preferences.preprocess.EOGchans(3) ~= -1
        eog.LLVEOG=EPmain.preferences.preprocess.EOGchans(3);
    else
        [eog.LLVEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'LLVEOG', EPtemplate.EPdata.montage);
    end
    if EPmain.preferences.preprocess.EOGchans(4) ~= -1
        eog.RLVEOG=EPmain.preferences.preprocess.EOGchans(4);
    else
        [eog.RLVEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'RLVEOG', EPtemplate.EPdata.montage);
    end
    if EPmain.preferences.preprocess.EOGchans(5) ~= -1
        eog.LHEOG=EPmain.preferences.preprocess.EOGchans(5);
    else
        [eog.LHEOG, ~] = ep_findChan(EPtemplate.EPdata.eloc, EPtemplate.EPdata.implicit, EPtemplate.EPdata.chanNames, EPtemplate.EPdata.ced, [], 'F9', EPtemplate.EPdata.montage);
    end
    if EPmain.preferences.preprocess.EOGchans(6) ~= -1
...
%It should contain only channels to be plotted, so no bad channels and no fiducials.
tempFlag=0;
...
[eeglabPath,A,ext] = fileparts(which('eeglab'));
...
if isempty(which([name '.spl']))
    ...
    if isempty(EPtemplate.EPdata.eloc)
        return
    end
    dataEloc=EPtemplate.EPdata.eloc;
%     dataEloc=[dataEloc EPtemplate.EPdata.implicit];
    chanList=zeros(length(dataEloc),1);
    for iChan=1:length(dataEloc)
        if ~isempty(dataEloc(iChan).X) && ~ismember(iChan,EPtemplate.badChans)
            chanList(iChan)=1;
...
[hdaxis cbaraxis] = headplot(theData,which([name '.spl']),'cbar',0,'maplimits',[min(theData) max(theData)],'labels',2,'meshfile',theMesh);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Confirms closing of the Template pane.
function confirmCloseTemplate(src,event)
