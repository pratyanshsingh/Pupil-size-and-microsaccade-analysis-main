...
global EPchunk EPtictoc EPmain
    ...
    for trial=1:numTrials
        ...
        if badTrialNum(theSubject,trial+trialCount-1)
            ...
            if ~isempty(trialBadChans) && ~isempty(trialGoodChans) && (dataChunk.avgNum(theSubject,realTrial)~=-1)  
                ...
                numEpochSamples=size(theData,2);
                if ft_hastoolbox('DISTCOMP') && (EPmain.preferences.advanced.parallel==1)
                    for theBadchan=1:length(trialBadChans)
                        repChan=zeros(numEpochSamples,1);
                        badchan=trialBadChans(theBadchan);
                        parfor iSample=1:numEpochSamples
                            [Xi,Yi,Zi] = griddata(x(trialGoodChans),y(trialGoodChans),theData(trialGoodChans,iSample),[1:GRID_SCALE]',[1:GRID_SCALE],'v4');
                            ...
                            %would just be NaN.
                            repChan(iSample)=Zi(y(badchan),x(badchan));
                        end
                        if (theBadchan==1) && (trial==1)
                            fprintf(repmat(sprintf('\b'),1,58)) %delete message about starting the worker pool
                        end
                        theData(badchan,:)=repChan;
                    end
                else
                    for iSample=1:numEpochSamples
                        [Xi,Yi,Zi] = griddata(x(trialGoodChans),y(trialGoodChans),theData(trialGoodChans,iSample),[1:GRID_SCALE]',[1:GRID_SCALE],'v4');
                        %v4 interpolates to the edge of the box.  With other interpolation options, if a bad channel is at the edge of the montage then it
                        %would just be NaN.
                        for theBadchan=1:length(trialBadChans)
                            badchan=trialBadChans(theBadchan);
                            theData(badchan,iSample)=Zi(y(badchan),x(badchan));
                        end
                    end
                end
                %change badChan field to indicate those channels have been corrected.
                if strcmp(dataChunk.dataType,'average')
                    theNum=dataChunk.avgNum(theSubject,trialCount+trial-1);
                    if theNum<1
                        theNum=1;
                    end
                    dataChunk.analysis.badChans(theSubject,trialCount+trial-1,trialBadChans)=theNum;
                else
